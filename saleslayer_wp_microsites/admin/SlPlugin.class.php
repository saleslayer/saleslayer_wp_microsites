...
if (!defined('SLYR_connector_id'))  include_once SLYR__PLUGIN_DIR.'../settings.php';
if (!class_exists('Softclear_API')) require_once SLYR__PLUGIN_DIR.'admin/api/api_sc.php';
...
class SlPlugin extends Softclear_API {
	...
	public function sync ($connectorid, $privatekey) {
		...
		}else if (isset($sync_data['error']) && !isset($sync_data['error']['message'])){

			return array('error'=>array('message'=>'Error on sync data'));

		}else if (is_array($sync_data) && empty($sync_data)){
